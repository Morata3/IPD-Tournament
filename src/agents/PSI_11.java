...
public class PSI_11 extends Agent {

    private State state;
    private AID mainAgent;
    private int myId, opponentId;
    private ACLMessage msg;
    private agentAction lastOponentAction;
    ...
    protected void setup() {
        ...
        System.out.println("TFT Agent " + getAID().getName() + " is ready.");
    ...
    protected void takeDown() {
        ...
        ack_kill.setContent("Tftt Player " + getAID().getName() + " terminating.");
        send(ack_kill);
    }
                ...
                switch (state) {
                    ...
                    case s1AwaitingGame:
                        if (msg.getPerformative() == ACLMessage.INFORM) {
                            if (msg.getContent().startsWith("Id#")) { //Game settings updated
                                ...
                                trusted = true;
                                lastOponentAction = agentAction.C;
                    ...
                    case s2Round:
                        if (msg.getPerformative() == ACLMessage.REQUEST && msg.getContent().startsWith("Action")) {
                            ...
                            if(lastOponentAction.compareTo(agentAction.D)==0 && trusted) defections ++;
                            else if(trusted) defections --;

                            if(defections >= 2){
                                trusted = false;
                                action = agentAction.D;
                            }
                            else action = lastOponentAction;
                    ...
                    case s3AwaitingResult:
                        if (msg.getPerformative() == ACLMessage.INFORM && msg.getContent().startsWith("Results#")) {
                            //Process results
                            if(myId>opponentId) oponentIndex = 0;
                            else oponentIndex = 1;
                            String oponentAction = msg.getContent().split("#")[2].split(",")[oponentIndex];
                            lastOponentAction = agentAction.valueOf(oponentAction);
                            state = State.s2Round;
                        } else {
