...
public class RandomAgent extends Agent {
    ...
    protected void takeDown() {
        ...
        ack_kill.setContent("Random Player " + getAID().getName() + " terminating.");
        send(ack_kill);
    }
            ...
            if (msg != null) {
                ...
                switch (state) {
                    ...
                    case s2Round:
                        ...
                        //Else error
                        random.setSeed(new Timestamp(System.nanoTime()).getTime());
                        if (msg.getPerformative() == ACLMessage.REQUEST && msg.getContent().startsWith("Action")) {
                            ACLMessage msg = new ACLMessage(ACLMessage.INFORM);
                            msg.addReceiver(mainAgent);
                            msg.setContent("Action#" + getRandomAction(random.nextInt(2)));
	 ...
	 * @return an action
	 */
	public String getRandomAction(int action){
            if (action == 0) return "C";
            else if(action == 1) return "D";
