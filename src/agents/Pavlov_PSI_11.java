...
public class Pavlov_PSI_11 extends Agent {

    private State state;
    private AID mainAgent;
    private int myId, opponentId;
    private ACLMessage msg;
    private int lastPayOff;
    private agentAction action = agentAction.C;
    ...
    protected void setup() {
        ...
        System.out.println("Pavlov Agent " + getAID().getName() + " is ready.");
    ...
    protected void takeDown() {
        ...
        ack_kill.setContent("Pavlov Player " + getAID().getName() + " terminating.");
        send(ack_kill);
    }
                ...
                switch (state) {
                    ...
                    case s2Round:
                        if (msg.getPerformative() == ACLMessage.REQUEST && msg.getContent().startsWith("Action")) {
                            ...
                            if(round != 0) {
                                if (lastPayOff < 3) {
                                    if(lastAction.compareTo(agentAction.C) == 0) action = agentAction.D;
                                    else action = agentAction.C;
                                } else {
                                    action = lastAction;
                                }
                            }else round ++;
                    ...
                    case s3AwaitingResult:
                        if (msg.getPerformative() == ACLMessage.INFORM && msg.getContent().startsWith("Results#")) {
                            //Process results
                            if(myId>opponentId) myIndex = 1;
                            else myIndex = 0;
                            lastAction = action;
                            lastPayOff = Integer.parseInt(msg.getContent().split("#")[3].split(",")[myIndex]);
                            state = State.s2Round;
                        } else {
